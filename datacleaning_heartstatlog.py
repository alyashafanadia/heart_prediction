...
class HeartDiseaseApp(App):
    ...
    def predict(self, instance):
        try:
            ...
            if prediction[0] == 1:
                self.show_prediction_popup("Ada kemungkinan penyakit jantung.", "Saran:", "1. Pemantauan kesehatan berkala", "2. Kontrol berat badan dan pola makan", "3. Konsumsi obat sesuai petunjuk", bold_first_sentence=True)
            elif prediction[0] == 0:
                self.show_prediction_popup("Tidak ada kemungkinan penyakit jantung.", "Saran:", "1. Lanjutkan gaya hidup sehat", "2. Pantau faktor risiko penyakit jantung", "3. Terlibat dalam deteksi dini", bold_first_sentence=True)
    ...
    def show_prediction_popup(self, *messages, bold_first_sentence=False):
        box_layout = BoxLayout(orientation='vertical')

        for index, message in enumerate(messages):
            label = Label(text=message, font_size=20, bold=bold_first_sentence and index==0)
            box_layout.add_widget(label)

        popup = Popup(title='Hasil Prediksi', content=box_layout, size_hint=(None, None), size=(400, 400))
        popup.open()
